...
export function Button({ href, variant, className, children}: ButtonProps) {
    return (
        ...
        ${variant === "primary" || variant === "secondary" ? "bg-accent-500 text-white font-semibold hover:bg-accent-300"
                : variant === "ghost" ? "h-15 px-8 text-[14px] uppercase tracking-[1px] decorate-none border border-white-7 bg-white-1 transition-all duration-300 ease-in-out hover:bg-primary-500 hover:border-primary-500"
                    : ""}`}>
            {children}
        </Link>
    )
